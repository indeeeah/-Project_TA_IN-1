...
<body>
<div style="text-align: center; margin-top:70px;"><h2>게시물 목록</h2></div>
<table align="center" cellpadding="10" cellspacing="0" border="1" width="60%">
	<tr>
		<td colspan="4" style="border: none;">
			<form method="post" name="listForm" action="boardManagelist.do">
		...
		<td align="right" colspan="6" cellspacing="0" style="border: none;"><input class="btn-action" type="button" value="전체목록"
				onclick="window.location='boardManagelist.do'"> </td>
	</tr>
	...
	<!-- 글이 없을 경우 -->
		<c:if test="${listCount eq 0}">
			<tr>
				<td colspan="6" align="center"><br> <br> 등록된 게시물이 없습니다.<br> <br></td>
			</tr>
		</c:if>	
		<c:if test="${listCount ne 0}">
			<c:forEach var="vo" items="${list}" varStatus="status">
				<tr>
					...
					<td align="center">${vo.b_id}</td>
					<td align="center">${vo.m_id}</td>
					<td align="center">${vo.b_content}</td>
					<td align="center">${vo.b_date}</td>
					<td align="center"><a class="btn-action" 
					onclick="if(confirm('정말 삭제 처리 하시겠습니까?')){href='bManageDelete.do?b_id=${vo.b_id}'}">강제삭제</a></td>
				</tr>
			</c:forEach>
		</c:if>	
		...
		<tr align="center" height="20">
			<td colspan="6"><c:if test="${currentPage <= 1}">
				 [이전]&nbsp;
					...
					<c:url var="blistST" value="boardManagelist.do">
					...
					<c:if test="${p ne currentPage}">
						<c:url var="blistchk" value="boardManagelist.do">
					...
					<c:url var="blistEND" value="boardManagelist.do">
						<c:param name="page" value="${currentPage+1}" />
					</c:url>
