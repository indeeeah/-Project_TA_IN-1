...
public class BsnBoardReplyController {
	...
	public HashMap <String, Object> bbrList(Locale locale, Model model, HttpServletRequest request) {
	...
	public HashMap <String, Object> bbrrList(Locale locale, Model model, HttpServletRequest request) {
		HashMap<String, Object> result = new HashMap <String,Object>();
		String bb_id = request.getParameter("bb_id");
		BsnBoardReply bbr = new BsnBoardReply();
		bbr.setBb_id(bb_id);;
		List<BsnBoardReply> logList = bbrService.bbrrList(bb_id);
		if ( logList.isEmpty() ) {
			String Msg = "이력이 없습니다.";
			String Code = "1";
			result.put("Msg", Msg);
	...
	public HashMap <String, Object> bbrrInsert(BsnBoardReply bbr, Locale locale, Model model, HttpServletRequest request) {
		System.out.println("답글등록 컨트롤러");
		HashMap<String, Object> result = new HashMap <String,Object>();
		String m_id= "aaaa";
		bbr.setM_id(m_id);
		System.out.println("답글등록 아이디"+bbr.getM_id());
		System.out.println("답글등록 상위글"+bbr.getBb_topid());
		System.out.println("답글등록 내용"+bbr.getBb_info());
		try {
			result.put("ack", bbrService.insertBbrr(bbr));
			System.out.println("답글인서트완료");
			List<BsnBoardReply> logList = bbrService.bbrrList(bbr.getBb_topid());
